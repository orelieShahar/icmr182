...
public class XYErrorRenderer extends XYLineAndShapeRenderer {
    ...
    */
    ...
    /*
//after sliding v={line} lines 272-291
    public void drawItem(Graphics2D g2, XYItemRendererState state, Rectangle2D dataArea, PlotRenderingInfo info, XYPlot plot,
                         ValueAxis domainAxis, ValueAxis rangeAxis, XYDataset dataset, int series, int item, CrosshairState crosshairState, int pass) {
        if (pass == 0 && dataset instanceof IntervalXYDataset
                && getItemVisible(series, item)) {
    ...
    public void drawItem(Graphics2D g2, XYItemRendererState state, Rectangle2D dataArea, PlotRenderingInfo info, XYPlot plot,
        ...
        if (pass == 0 && dataset instanceof IntervalXYDataset
            ...
            if (this.drawXError) {
                ...
                g2.draw(getLine2D(dataArea, plot, domainAxis, rangeAxis, series, item, ixyd, orientation));
                g2.draw(cap1);
                g2.draw(cap2);
            }
            if (this.drawYError) {
...
//after sliding v={cap1}  result extract method after inline temp on cap1 and cap2
    ...
    public void drawItem(Graphics2D g2, XYItemRendererState state, Rectangle2D dataArea, PlotRenderingInfo info, XYPlot plot,
        ...
        if (pass == 0 && dataset instanceof IntervalXYDataset
            ...
            if (this.drawXError) {
                ...
                g2.draw(getLine2D(dataArea, plot, domainAxis, rangeAxis, series, item, ixyd, orientation));
                g2.draw(getCap1Line2D(dataArea, plot, domainAxis, rangeAxis, series, item, ixyd, orientation));
                g2.draw(getCap2Line2D(dataArea, plot, domainAxis, rangeAxis, series, item, ixyd, orientation));
            }
            if (this.drawYError) {
                // draw the error bar for the y-interval
                double y0 = ixyd.getStartYValue(series, item);
